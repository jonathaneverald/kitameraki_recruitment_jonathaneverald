...
export class UserService {
    ...
    static async login(request: LoginUserRequest): Promise<UserResponse> {
        try {
            ...
            const query = {
                query: "SELECT c.id, c.username, c.name, c.password, c.salt FROM c WHERE c.username = @username",
    ...
    static async update(user: User, request: UpdateUserRequest): Promise<UserResponse> {
        try {
            ...
            if (updateRequest.password) {
                ...
                // Compare the current password with the updated password
                const passwordMatches = await authService.comparePassword(
                    currentUser.password, // Stored hash
                    currentUser.salt, // Stored salt
                    updateRequest.password // Input password to check
                );

                if (!passwordMatches) {
                    // Hash the new password
                    const { salt, hash } = await authService.hashPassword(updateRequest.password);
                    updateData.password = hash;
                    updateData.salt = salt; // Store the new salt as well
                }
            }
